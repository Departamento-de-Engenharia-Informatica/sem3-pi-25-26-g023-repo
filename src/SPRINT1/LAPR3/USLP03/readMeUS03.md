...
### Simple Interaction Diagram (PlantUML)

```plantuml
@startuml
actor User
participant TravelTimeUI as UI
participant TravelTimeController as Controller
participant EstacaoRepository as EstRepo
participant LocomotivaRepository as LocoRepo
participant SegmentoLinhaRepository as SegRepo
participant RailwayNetworkService as NetService

User -> UI : Select Option 5
activate UI
UI -> EstRepo : findAll()
activate EstRepo
EstRepo --> UI : List<Estacao>
deactivate EstRepo
UI --> User : Display Stations
User -> UI : Select Departure ID
UI -> Controller : getDirectlyConnectedStations(depID)
activate Controller
Controller -> SegRepo : findAll() / findDirectSegment()
activate SegRepo
SegRepo --> Controller : List<SegmentoLinha> / Filter logic
deactivate SegRepo
Controller -> EstRepo : findById(connectedID)
activate EstRepo
EstRepo --> Controller : Optional<Estacao>
deactivate EstRepo
Controller --> UI : List<Estacao> (connected)
deactivate Controller
UI --> User : Display Connected Stations
User -> UI : Select Arrival ID
UI -> LocoRepo : findAll()
activate LocoRepo
LocoRepo --> UI : List<Locomotiva>
deactivate LocoRepo
UI --> User : Display Locomotives
User -> UI : Select Locomotive ID
UI -> Controller : calculateTravelTime(depID, arrID, locoID)
activate Controller
Controller -> EstRepo : findById()
Controller -> LocoRepo : findById()
Controller -> NetService : findFastestPath(depID, arrID)
activate NetService
NetService -> SegRepo : findAll() ' Gets edges and weights
activate SegRepo
SegRepo --> NetService : List<SegmentoLinha>
deactivate SegRepo
NetService -> NetService : Run Dijkstra Algorithm
NetService --> Controller : RailwayPath
deactivate NetService
Controller -> Controller : formatPathResult(path, ...)
Controller --> UI : Formatted Result String
deactivate Controller
