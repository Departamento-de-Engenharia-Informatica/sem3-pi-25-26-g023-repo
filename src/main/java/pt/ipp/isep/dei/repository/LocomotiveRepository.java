...
public class LocomotiveRepository {
    ...
    public List<Locomotive> findAll() { /* ... código JDBC ... */
        ...
        try (Connection conn = DatabaseConnection.getConnection(); PreparedStatement stmt = conn.prepareStatement(sql); ResultSet rs = stmt.executeQuery()) {
            while (rs.next()) { /* ... Mapeamento ... */
                ...
                try { int stockIdInt = Integer.parseInt(stockIdStr); locomotivas.add(new Locomotive(stockIdInt, model, tipoJava)); //
    ...
    public Optional<Locomotive> findById(int id) { /* ... código JDBC ... */
        ...
        try (Connection conn = DatabaseConnection.getConnection(); PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) { /* ... Mapeamento ... */
                    String stockIdStr = rs.getString("stock_id"); String model = rs.getString("model"); String typeDb = rs.getString("type");
                    String tipoJava = mapTypeToDomain(typeDb);
                    try { int stockIdInt = Integer.parseInt(stockIdStr); locomotiva = new Locomotive(stockIdInt, model, tipoJava); //
